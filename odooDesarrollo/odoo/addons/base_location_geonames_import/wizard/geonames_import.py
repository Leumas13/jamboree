...
class CityZipGeonamesImport(models.TransientModel):
    ...
    def get_and_parse_csv(self, country):
        ...
        # Ruta al ZIP local en tu módulo
        zip_path = get_module_resource(
            'base_location_geonames_import', 'static', 'geonames', f"{country_code}.zip"
        )
        
        if not zip_path or not os.path.isfile(zip_path):
            raise UserError(_("No se encontró el archivo ZIP para el país %s.") % country_code)

        logger.info("Usando ZIP local: %s", zip_path)

        # Abrir el ZIP local
        with zipfile.ZipFile(zip_path, 'r') as f_geonames:
            if f"{country_code}.txt" not in f_geonames.namelist():
                raise UserError(_("El archivo %s.txt no está dentro del ZIP.") % country_code)

            # Extraer archivo temporalmente y leer
            with f_geonames.open(f"{country_code}.txt") as data_file:
                reader = csv.reader(io.TextIOWrapper(data_file, encoding='utf-8'), delimiter="\t")
                parsed_csv = [row for row in reader]

        logger.info("El archivo %s.txt ha sido leído correctamente desde el ZIP local.", country_code)
        return parsed_csv
