...
class Entrenamiento(models.Model):
    ...
    # Campos relacionados con la sede
    ...
    # Campos relacionados con el entrenador
    ...
    # Campos relacionados con los jugadores
    ...
    mostrar_ficha = fields.Boolean('Mostrar ficha', compute='_compute_mostrar_ficha', store=False)

    # Mostrar ficha cuando quedan menos de 2 horas
    @api.depends('turno')
    def _compute_mostrar_ficha(self):
        for record in self:
            if record.turno:
                tiempo_restante = record.turno - datetime.now()
                record.mostrar_ficha = tiempo_restante.total_seconds() <= 7200
            else:
                record.mostrar_ficha = False

    # Buscar el uid de usuario de los entrenadores
    @api.depends('entrenador_ids.user_id')
    def _compute_user_ids(self):
        for record in self:
            record.user_ids = record.entrenador_ids.mapped('user_id').filtered(lambda u: u)

    # Semáforo para el color de ocupación
    ...
    # Comprobar si el entrenamiento ha terminado        
    ...
    # Actualizar el evento del entrenamiento
    ...
    # Crear o actualizar el evento del entrenamiento
